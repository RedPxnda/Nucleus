 ...
 * it's recommended to use the {@link com.redpxnda.nucleus.widgets.widgets.NucleusWidget} class.
 *
 * @param <T> the type of the screen handler associated with this screen
 */
@SuppressWarnings("unused")
public abstract class NucleusScreen<T extends AbstractContainerMenu> extends AbstractContainerScreen<T> {
     ...
     */
    protected NucleusScreen(T handler, Inventory inventory, Component title) {
    ...
    public void removeChild(GuiEventListener element) {
        ...
        if (element instanceof BasicHirarchyWidget basicHirarchyWidget) {
            basicHirarchyWidget.setParent((Screen) null);
        }
    }
    ...
    public void renderHover(GuiGraphics context, int mouseX, int mouseY, float delta) {
        ...
        children().forEach(element -> {
            if (element instanceof BasicAttachAbleWidget basicAttachAbleWidget) {
                basicAttachAbleWidget.renderHover(context, mouseX, mouseY, delta);
            }
        });
    }
}
