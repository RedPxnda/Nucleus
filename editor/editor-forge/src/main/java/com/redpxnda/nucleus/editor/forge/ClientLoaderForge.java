...
import com.redpxnda.nucleus.editor.diagnostic.ImGuiDebugPanels;
...
public class ClientLoaderForge {
	...
	public static void registerOverlays(RegisterGuiLayersEvent event) {
		event.registerBelowAll(ResourceLocation.parse("nucleus_editor:debug_tick"), ImGuiDebugPanels::onRender);
	}
	...
	private static void setupImGuiLibARM64() {
		...
		try {
			System.loadLibrary("imgui-javaarm64"); // or "imgui-java64" for Windows
			System.out.println("Native ImGui library loaded successfully!");
		} catch (UnsatisfiedLinkError e) {
			System.err.println("Failed to load native ImGui library: " + e.getMessage());
		}
		var loadedLibraries = ClassLoader.getSystemClassLoader().getDefinedPackages();
		NucleusEditor.LOGGER.info("loaded lib");
	}
}
