...
public class Transform {
    ...
    public static final Codec<Vector3f> VECTOR_CODEC = Codec.withAlternative(
            RecordCodecBuilder.create(instance -> instance.group(
                    Codec.FLOAT.optionalFieldOf("x", 0.0f).forGetter(Vector3f::x),
                    Codec.FLOAT.optionalFieldOf("y", 0.0f).forGetter(Vector3f::y),
                    Codec.FLOAT.optionalFieldOf("z", 0.0f).forGetter(Vector3f::z)
            ).apply(instance, Vector3f::new)),
            Codec.list(Codec.DOUBLE).xmap(
                    list -> new Vector3f(
    ...
    private static void validateMatrix(Matrix4f matrix) {
        if (!matrix.isFinite()) {
            Nucleus.getLogger().error("Invalid transform matrix contains NaN or Infinity: {}", matrix);
            return;
        }

        float determinant = matrix.determinant();
        if (Math.abs(determinant) < 1.0E-6f) {
            Nucleus.getLogger().error("Invalid transform matrix has zero determinant: {}", matrix);
            return;
        }
    ...
    public static Matrix4f toMatrix(Vector3f rotation, Vector3f translation, Vector3f scale) {
        Vector3f fixedRotation = fixEuler(rotation);
        ...
        return new Matrix4f()
                ...
                .rotate((new Quaternionf()).rotationXYZ(
                        fixedRotation.x() * 0.017453292F,
                        fixedRotation.y() * 0.017453292F,
                        fixedRotation.z() * 0.017453292F))
                .mul(scaleMatrix);
    }
    ...
    public int[] rotateVertexData(int[] vertexData) {
        ...
        for (int i = 0; i < rotatedData.length; i += 8) {
            ...
            if (packedNormal != 0) {
                ...
                int rx = Math.clamp(Math.round(normal.x * 127.0f), -127, 127);
                int ry = Math.clamp(Math.round(normal.y * 127.0f), -127, 127);
                int rz = Math.clamp(Math.round(normal.z * 127.0f), -127, 127);
     ...
     * currently doesn't support rotation or scale.
     */
    ...
    public static class TransformJsonAdapter extends TypeAdapter<Transform> {
        ...
        public Transform read(JsonReader jsonReader) throws IOException {
            ...
            while (jsonReader.hasNext()) {
                ...
                switch (name) {
                    case "origin" -> origin = jsonReader.nextString();
                    case "rotation" -> rotation = readVector3f(jsonReader);
                    case "translation" -> translation = readVector3f(jsonReader);
                    case "scale" -> scale = readVector3f(jsonReader);
                    case null, default -> jsonReader.skipValue();
                }
            }
    ...
    /**
     * Interpolates between two transforms without interpolating Euler angles.
     *
     * @param a the starting transform
     * @param b the ending transform
     * @param t interpolation factor, normally in the range [0, 1]
     * @return the interpolated transform
     */
    public static Transform interpolate(Transform a, Transform b, float t) {
        t = Math.clamp(t,0.0f,1.0f);

        Vector3f translationA = a.getTranslation();
        Vector3f translationB = b.getTranslation();

        Vector3f scaleA = a.getScale();
        Vector3f scaleB = b.getScale();

        Vector3f translation = new Vector3f(translationA).lerp(translationB, t);
        Vector3f scale = new Vector3f(scaleA).lerp(scaleB, t);

        Quaternionf rotationA = new Quaternionf().setFromNormalized(new Matrix3f(a.matrix));
        Quaternionf rotationB = new Quaternionf().setFromNormalized(new Matrix3f(b.matrix));

        rotationA.slerp(rotationB, t);
        ...
        return new Transform(result).withOrigin(
                t < 0.5f ? a.origin : b.origin
        );
    }

    /**
     * Interpolates this transform towards another transform.
     *
     * @param other the target transform
     * @param t     interpolation factor, normally in the range [0, 1]
     * @return the interpolated transform
     */
    public Transform interpolate(Transform other, float t) {
